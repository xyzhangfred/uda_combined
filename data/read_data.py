...
import pandas as pd
def read_imdb_cf_data(prefix = 'train', data_dir = DATA_DIR, max_len = 128, num = 100):
...
def read_matres_data(prefix = 'train', data_dir = '/home/xiongyi/dataxyz/data/contrast-sets/', max_len = 128, num = 100):
...
def get_sup_dataset(tokenizer,orig_sents,orig_labels,cf_sents, cf_labels, batch_size = 8,max_len=128):
...
def get_eval_dataset(tokenizer,orig_sents,orig_labels,cf_sents, cf_labels, batch_size = 8,max_len=128):
    orig_examples = [InputExample(guid= str(i), text_a = sent, label= orig_labels[i]) for i,sent in enumerate(orig_sents)]
    cf_examples = [InputExample(guid= str(i), text_a = sent, label= cf_labels[i]) for i,sent in enumerate(cf_sents)]
    orig_features = convert_examples_to_features(orig_examples, tokenizer, max_length=max_len, )
    cf_features = convert_examples_to_features(cf_examples, tokenizer, max_length=max_len, )
