...
def subset_baseline_cf(source_path, new_dir,data_num= 100):
...
def subset_baseline_matres(source_path, new_dir,data_num= 100):
    out_dir = new_dir.format(data_num)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
...
def subset_baseline_mctaco(source_path, new_dir,data_num= 100):
    out_dir = new_dir.format(data_num)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
...
def main():
    ...
    for num in [200,100,50,20,10,5,2]:
        if prev_num is None:
            if args.data_type == 'cf':
                subset_baseline_cf(data_num=num, source_path = args.source_path, new_dir = new_dir)
            elif args.data_type == 'matres':
                subset_baseline_matres(data_num=num, source_path = args.source_path, new_dir = new_dir)
            elif args.data_type == 'mctaco':
                subset_baseline_mctaco(data_num=num, source_path = args.source_path, new_dir = new_dir)
    
        else:
            if args.data_type == 'cf':
                subset_baseline_cf(data_num=num, source_path = new_dir.format(prev_num), new_dir = new_dir)
            elif args.data_type == 'matres':
                subset_baseline_matres(data_num=num, source_path = new_dir.format(prev_num), new_dir = new_dir)
            elif args.data_type == 'mctaco':
                subset_baseline_mctaco(data_num=num, source_path = new_dir.format(prev_num), new_dir = new_dir)
