...
class load_data:
    ...
    def eval_data_iter(self):
        if self.data_type == 'imdb_cf':
            orig_eval_sents,orig_eval_labels, cf_eval_sents, cf_eval_labels = read_imdb_cf_data(prefix = 'dev', data_dir = self.eval_data_dir, max_len = 128)
        elif self.data_type == 'imdb_contrast':
            #for imdb_contrast use test data as training data since there's more of that
            orig_eval_sents,orig_eval_labels, cf_eval_sents, cf_eval_labels = read_imdb_contrast_data(prefix = 'dev', max_len = 128,data_dir = self.eval_data_dir,num=100)
        elif self.data_type == 'matres':
