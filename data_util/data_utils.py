...
def paired_data_collator(paired_features):
    ...
    batch['cf']["labels"] = torch.tensor([f[1]["label"] for f in paired_features], dtype=int)
    ...
    for k, v in first.items():
        if k not in ("label", "label_ids") and v is not None and not isinstance(v, str):
            if isinstance(v, torch.Tensor):
                ...
                batch['cf'][k] = torch.stack([f[1][k] for f in paired_features])
            else:
                batch['orig'][k] = torch.stack([torch.LongTensor(f[0][k]) for f in paired_features])
                batch['cf'][k] = torch.stack([torch.LongTensor(f[1][k]) for f in paired_features])
...
def get_paired_dataloader(tokenizer,orig_sents,orig_labels,cf_sents, cf_labels, batch_size = 8,max_len=128, text_b = False):
    if not text_b:
...
def get_single_dataloader(tokenizer,sents,labels, batch_size = 8,max_len=128,shuffle = False,text_b = False):
    if not text_b:
        examples = [InputExample(guid= str(i), text_a = sent, label= labels[i]) for i,sent in enumerate(sents)]
    else:
        examples = [InputExample(guid= str(i), text_a = sent.split('\t\t')[0],text_b = sent.split('\t\t')[1], label= labels[i]) for i,sent in enumerate(sents)]
