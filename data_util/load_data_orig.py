...
from utils_orig import truncate_tokens_pair
...
class load_data:
    def __init__(self, cfg):
        ...
        if cfg.mode == 'train':
            ...
            self.shuffle = True
        elif cfg.mode == 'train_eval_cf':
            self.sup_data_dir = cfg.sup_data_dir
            self.orig_eval_data_dir= cfg.orig_eval_data_dir
            self.cf_eval_data_dir= cfg.cf_eval_data_dir
            self.sup_batch_size = cfg.train_batch_size
            self.eval_batch_size = cfg.eval_batch_size
            self.shuffle = True
        elif cfg.mode == 'eval':
    ...
    def orig_eval_data_iter(self):
        orig_eval_dataset = self.TaskDataset(self.orig_eval_data_dir, self.cfg.need_prepro, self.pipeline, self.cfg.max_seq_length, 'orig_eval', 'sup')
        orig_eval_data_iter = DataLoader(orig_eval_dataset, batch_size=self.eval_batch_size, shuffle=False)

        return orig_eval_data_iter

    def cf_eval_data_iter(self):
        cf_eval_dataset = self.TaskDataset(self.cf_eval_data_dir, self.cfg.need_prepro, self.pipeline, self.cfg.max_seq_length, 'cf_eval', 'sup')
        cf_eval_data_iter = DataLoader(cf_eval_dataset, batch_size=self.eval_batch_size, shuffle=False)

        return cf_eval_data_iter
